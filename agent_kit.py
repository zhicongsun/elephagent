...
1. Edit `.agent/memory/*.md` or run `python3 agent_kit.py remember "..."`.
2. Run `python3 agent_kit.py build`.
...
def generated_notice() -> str:
    return "> Generated by `python3 agent_kit.py build`. Edit `.agent/` instead of this file."
...
- Add durable discoveries with `python3 agent_kit.py remember "..."`, then run `python3 agent_kit.py build`.
...
- Add lasting notes with `python3 agent_kit.py remember "..."`, then run `python3 agent_kit.py build`.
...
def render_codex_toml(registry: dict[str, Any]) -> str:
    lines = [
        "# Generated by `python3 agent_kit.py build`. Edit `.agent/tools/registry.json` instead.",
        "",
    ]
    for name, server in registry.get("mcpServers", {}).items():
...
def build_command(args: argparse.Namespace) -> int:
    ...
    if not (root / ".agent").exists():
        print("No .agent directory found. Run `python3 agent_kit.py init` first.", file=sys.stderr)
        return 1
...
def sync_command(args: argparse.Namespace) -> int:
    ...
    if not (root / ".git").exists():
        print("This folder is not a Git repository. Run `git init` and add a remote before `agent_kit.py sync`.", file=sys.stderr)
        return 1
    build_command(args)
...
def doctor_command(args: argparse.Namespace) -> int:
    ...
    for key, path_name in config.get("generated", GENERATED_PATHS).items():
        ...
        if path.exists():
            ...
            warn(f"{path_name} is missing; run `python3 agent_kit.py build`")
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"agent-kit {VERSION}")
