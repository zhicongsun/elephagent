...
def sync_command(args: argparse.Namespace) -> int:
    ...
    if not (root / ".git").exists():
        print("This folder is not a Git repository. Run `git init` and add a remote before `agentmem.py sync`.", file=sys.stderr)
        return 1
    build_command(args)
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"agentmem {VERSION}")
