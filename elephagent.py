...
DEFAULT_SKILLS: dict[str, str] = {
    ...
    "el-handoff": DEFAULT_HANDOFF_SKILL,
}
...
def strip_yaml_frontmatter(content: str) -> str:
    lines = content.lstrip().splitlines(True)
...
def is_elephagent_generated(content: str) -> bool:
    return "Generated by" in content[:500] and "elephagent" in content[:500]


def parse_memory_snapshot(content: str) -> dict[str, str]:
...
def import_skill_from_dir(skill_dir: Path, root: Path, default_skill_names: set[str], label: str) -> tuple[list[str], list[str]]:
    """Import skills from a directory. Returns (imported, skipped) lists."""
    imported: list[str] = []
    skipped: list[str] = []
    if not skill_dir.exists():
        return imported, skipped

    # Directory contains SKILL.md files in subdirs (Claude format)
    for child in sorted(skill_dir.iterdir()):
        skill_file = child / "SKILL.md" if child.is_dir() else None
        md_file = child if child.is_file() and child.suffix in (".md", ".mdc") else None
        source_file = skill_file if skill_file and skill_file.exists() else md_file
        if not source_file:
            continue
        name = child.stem if child.is_file() else child.name
        if name in default_skill_names or name == "agent-memory":
            continue
        content = source_file.read_text(encoding="utf-8")
        if is_elephagent_generated(content):
            continue
        body = strip_yaml_frontmatter(content)
        dest = root / ".agent" / "skills" / f"{name}.md"
        if not dest.exists():
            dest.parent.mkdir(parents=True, exist_ok=True)
            write_text(dest, body.strip() + "\n")
            imported.append(f"{name} (from {label})")
        else:
...
def import_command(args: argparse.Namespace) -> int:
    root = find_root()
    if not (root / ".agent").exists():
        print("No .agent directory found. Running init first...")
        init_args = argparse.Namespace(force=False, no_build=True)
        init_command(init_args)

    source = args.source
    imported_memories: list[str] = []
    imported_skills: list[str] = []
    skipped: list[str] = []
    default_skill_names = set(DEFAULT_SKILLS.keys())

    # --- Import from --path or positional skill names ---
    external_path = getattr(args, "path", None)
    skill_names = getattr(args, "names", None) or []

    if external_path:
        ext = Path(external_path).expanduser().resolve()
        if not ext.exists():
            print(f"Path not found: {ext}", file=sys.stderr)
            return 1
        imp, skp = import_skill_from_dir(ext, root, default_skill_names, str(ext))
        imported_skills.extend(imp)
        skipped.extend(skp)
    ...
    if skill_names:
        for name in skill_names:
            ...
            for gdir in GLOBAL_SKILL_DIRS:
                # Try as subdirectory with SKILL.md
                skill_dir = gdir / name
                if skill_dir.is_dir() and (skill_dir / "SKILL.md").exists():
                    ...
                    if not dest.exists():
                        dest.parent.mkdir(parents=True, exist_ok=True)
                        write_text(dest, body.strip() + "\n")
                        imported_skills.append(f"{name} (from {gdir})")
                    else:
                        skipped.append(f"skill {name} (already exists)")
                    found = True
                    break
                # Try as .md file
                skill_file = gdir / f"{name}.md"
                if skill_file.is_file():
                    ...
                    break
            if not found:
                print(f"Skill '{name}' not found in global directories.", file=sys.stderr)

    # --- If --path or names provided, skip project-level scan ---
    if external_path or skill_names:
        if imported_skills:
    ...
    if source in ("auto", "claude"):
        ...
        if claude_md.exists():
            ...
            if not is_elephagent_generated(content):
                dest = root / ".agent" / "memory" / "imported-claude.md"
                if not dest.exists():
                    dest.parent.mkdir(parents=True, exist_ok=True)
                    write_text(dest, f"# Imported from CLAUDE.md\n\n{content.strip()}\n")
                    imported_memories.append("imported-claude.md (from CLAUDE.md)")
                else:
                    skipped.append("CLAUDE.md (already imported)")

    # Cursor: .cursor/rules/agent-memory.mdc
    if source in ("auto", "cursor"):
        cursor_mem = root / ".cursor" / "rules" / "agent-memory.mdc"
        if cursor_mem.exists():
            content = cursor_mem.read_text(encoding="utf-8")
            if not is_elephagent_generated(content):
                body = strip_yaml_frontmatter(content)
                snapshot_idx = body.find("## Memory Snapshot")
                if snapshot_idx >= 0:
                    snapshot = body[snapshot_idx:]
                    sections = parse_memory_snapshot(snapshot)
                    for path, text in sections.items():
                        dest = root / ".agent" / "memory" / Path(path).name
                        if not dest.exists():
                            dest.parent.mkdir(parents=True, exist_ok=True)
                            write_text(dest, text + "\n")
                            imported_memories.append(f"{dest.name} (from cursor agent-memory.mdc)")
                        else:
                            skipped.append(f"{dest.name} (already exists)")
                else:
                    dest = root / ".agent" / "memory" / "imported-cursor.md"
                    if not dest.exists():
                        write_text(dest, f"# Imported from Cursor rules\n\n{body.strip()}\n")
                        imported_memories.append("imported-cursor.md (from agent-memory.mdc)")
    ...
    if source in ("auto", "codex"):
        ...
        if agents_md.exists():
            ...
            if not is_elephagent_generated(content):
                snapshot_idx = content.find("## Memory Snapshot")
                if snapshot_idx >= 0:
                    snapshot = content[snapshot_idx:]
                    sections = parse_memory_snapshot(snapshot)
                    for path, text in sections.items():
                        dest = root / ".agent" / "memory" / Path(path).name
                        if not dest.exists():
                            dest.parent.mkdir(parents=True, exist_ok=True)
                            write_text(dest, text + "\n")
                            imported_memories.append(f"{dest.name} (from AGENTS.md)")
                        else:
                            skipped.append(f"{dest.name} (already exists)")
                else:
                    dest = root / ".agent" / "memory" / "imported-codex.md"
                    if not dest.exists():
                        write_text(dest, f"# Imported from AGENTS.md\n\n{content.strip()}\n")
                        imported_memories.append("imported-codex.md (from AGENTS.md)")

    # --- Skill Import (project-level) ---

    # Claude Code: .claude/skills/*/SKILL.md
    if source in ("auto", "claude"):
        imp, skp = import_skill_from_dir(root / ".claude" / "skills", root, default_skill_names, ".claude/skills/")
        imported_skills.extend(imp)
        skipped.extend(skp)

    # Cursor: .cursor/rules/*.mdc (excluding agent-memory.mdc)
    if source in ("auto", "cursor"):
        cursor_rules = root / ".cursor" / "rules"
        if cursor_rules.exists():
            for mdc_file in sorted(cursor_rules.glob("*.mdc")):
                name = mdc_file.stem
                if name == "agent-memory" or name in default_skill_names:
                    continue
                ...
                if not dest.exists():
                    dest.parent.mkdir(parents=True, exist_ok=True)
                    write_text(dest, body.strip() + "\n")
                    imported_skills.append(f"{name} (from .cursor/rules/)")
                else:
                    skipped.append(f"skill {name} (already exists)")
    ...
    if source == "auto":
        for gdir in GLOBAL_SKILL_DIRS:
            if gdir.exists():
                imp, skp = import_skill_from_dir(gdir, root, default_skill_names, str(gdir))
                imported_skills.extend(imp)
                skipped.extend(skp)
    ...
    if skipped:
        ...
        for item in skipped:
            print(f"  {item}")
    if not imported_memories and not imported_skills:
        print("Nothing new to import.")
        return 0
...
def make_parser() -> argparse.ArgumentParser:
    ...
    imp = sub.add_parser("import", help="Import existing memories and skills from Claude Code, Cursor, or Codex.")
    imp.add_argument("names", nargs="*", help="Skill names to import from global directories (~/.cursor/skills/, ~/.claude/skills/).")
    imp.add_argument("--from", dest="source", default="auto", choices=["auto", "claude", "cursor", "codex"])
    imp.add_argument("--path", help="Path to a directory containing skills to import.")
    imp.add_argument("--no-build", action="store_true", help="Skip rendering platform adapter files after import.")
    imp.set_defaults(func=import_command)
