...
- The user has an existing project with hand-written `CLAUDE.md`, `.cursor/rules/`, or custom skills and wants to centralize them

## Steps

1. Run `elephagent import` in the project root (auto-detects all platforms)
   - If the user specifies a platform, use `elephagent import --from claude`, `--from cursor`, or `--from codex`
2. Read the output and summarize:
        ...
        if claude_skills.exists():
            for skill_dir in sorted(claude_skills.iterdir()):
                ...
                if not dest.exists():
                    ...
                    imported_skills.append(f"{name} (from .claude/skills/)")
                else:
